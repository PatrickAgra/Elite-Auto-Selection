...
if (isset($_POST['add_product'])) {
    ...
    $image_folder = 'db_auto_images/' . $image;

    $select_products = $pdo->prepare("SELECT * FROM `assortiment` WHERE assortiment = ?");
    $select_products->execute([$product_name]);

    if ($select_products->rowCount() > 0) {
        echo '<div class="error"><p>product is al toegevoegd</p></div>';
    } else {
        $insert_products = $pdo->prepare("INSERT INTO `assortiment`(product, jaartal, kilometers, prijs, omschrijving, foto1, foto2, foto3)
        VALUES(?,?,?,?,?,?,?,?)");
